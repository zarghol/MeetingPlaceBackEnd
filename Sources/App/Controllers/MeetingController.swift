...
final class MeetingController {
    ...
    func mine(_ req: Request) throws -> Future<[Meeting]> {
        ...
        return try user.meetings.query(on: req).all()
    }
    ...
    func oneDay(_ req: Request) throws -> Future<[Meeting]> {
        ...
        return Meeting.query(on: req).filter(\.presentationDate, .equal, date).all()
    }
        ...
        return Meeting
            ...
            .flatMap {
                if var modified = $0.0 {
                    ...
                    return modified.update(on: req).map { _ in HTTPResponse(status: .ok) }
                } else {
